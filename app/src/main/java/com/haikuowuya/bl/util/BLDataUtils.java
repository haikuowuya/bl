                ...
                if (null != trElements && !trElements.isEmpty())
                {
                    if (trElements.size() > 1)
                    {
                        for (Element tmpElement : trElements)
                        {
                            ...
                            {
                               // SoutUtils.out("tdElements = " + tdElements);
                                ...
                                Element aElement = tdElements.get(0).getElementsByTag("a").get(0);
                                lineName = aElement.text();
                                lDirection = tdElements.get(1).text();
                                busCard = tdElements.get(2).text();
                                inTime = tdElements.get(3).text();
                                distance = tdElements.get(4).text();
                                /**/
                                SoutUtils.out("lineName = " + lineName+ " lDirection = " + lDirection + " busCard = " + busCard+" inTime = " + inTime
                                +" distance = "+distance
                                );

                                stopItem.SName = lineName;
                                stopItem.LDirection = lDirection;
                                stopItem.InTime = inTime;
                                stopItem.DBusCard = busCard;
                                stopItem.Distince = distance;
